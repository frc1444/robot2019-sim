...
public class AutonomousModeCreator {
	...
	public Action createAction(
		...
		if (autonomousType == AutonomousType.DO_NOTHING) {
			...
			actionQueue.add(actionCreator.createGoStraight(inchesToMeters(55), .5, Rotation2.DEG_90)); // cross line
			...
			actionQueue.add(actionCreator.createGoStraight(inchesToMeters(65), .5, isLeft ? Rotation2.DEG_180 : Rotation2.ZERO)); // go towards wall
			actionQueue.add(actionCreator.createGoStraight(inchesToMeters(55), .5, Rotation2.DEG_90)); // cross line
			...
			// go 100 inches
			actionQueue.add(actionCreator.createGoStraight(inchesToMeters(40.0), .3, Rotation2.DEG_90, startingOrientation)); // get off hab
			actionQueue.add(actionCreator.createGoStraight(inchesToMeters(30.0), .7, Rotation2.DEG_90, startingOrientation)); // drive a little
			actionQueue.add(actionCreator.createGoStraight(inchesToMeters(30.0), .3, Rotation2.DEG_90, startingOrientation)); // drive slower
			// went 100 inches
			assert gamePieceType == GamePieceType.HATCH : "always true";
			final Rotation2 faceAngle = getManipulatorOffset(GamePieceType.HATCH).plusDegrees(90); // face the manipulator towards the cargo ship
			...
			if(lineUpType == LineUpType.USE_VISION) {
				...
				actionQueue.add(new Actions.ActionMultiplexerBuilder(
						...
						actionCreator.createGoStraight(inchesToMeters(10.0), .3, faceAngle, faceAngle) // go forward
				).build());
				final double hatchExtend = dimensions.getHatchManipulatorActiveExtendDistanceMeters();
				if(hatchExtend < inchesToMeters(20)) {
					actionQueue.add(actionCreator.createGoStraight(inchesToMeters(20) - hatchExtend, .15, faceAngle, faceAngle));
				}
				actionQueue.add(actionCreator.createDropHatch());
			}
			if(afterComplete != null){
				actionQueue.add(actionCreator.createGoStraight(inchesToMeters(20), .3, Rotation2.DEG_270, faceAngle));
				if(afterComplete == AfterComplete.PREPARE_FOR_DEFENSE){
					actionQueue.add(actionCreator.createGoStraight(inchesToMeters(50), .3, isLeft ? Rotation2.DEG_180 : Rotation2.ZERO, faceAngle));
					actionQueue.add(actionCreator.createGoStraight(inchesToMeters(50), .3, Rotation2.DEG_90, faceAngle));
				} else {
					final boolean hatch;
					...
					actionQueue.add(actionCreator.createGoStraight(inchesToMeters(70), .3, isLeft ? Rotation2.DEG_180 : Rotation2.ZERO, faceAngle));
					...
					actionQueue.add(actionCreator.createGoStraight(inchesToMeters(100), .3, Rotation2.DEG_270, faceDirection));
				}
			}
			...
			actionQueue.add(actionCreator.createGoStraight(inchesToMeters(40.0), .3, Rotation2.DEG_90, startingOrientation));
			distance -= inchesToMeters(40);
			actionQueue.add(actionCreator.createGoStraight(inchesToMeters(115.0) / longDistanceAngle.getSin(), .7, longDistanceAngle, startingOrientation));
			distance -= inchesToMeters(115);
			actionQueue.add(actionCreator.createGoStraight(distance, .3, Rotation2.DEG_90, startingOrientation));
			...
			if(lineUpType == LineUpType.USE_VISION) {
				...
				final Action driveAction = actionCreator.createGoStraight(inchesToMeters(20.0), .2, towardsCargoShipAngle, faceAngle);
				if(gamePieceType == GamePieceType.HATCH){
					actionQueue.add(new Actions.ActionMultiplexerBuilder(
			...
			if(afterComplete != null){
				successQueue.add(actionCreator.createGoStraight(inchesToMeters(30.0), .4, towardsCargoShipAngle.unaryMinus(), faceAngle));
				if(afterComplete == AfterComplete.PREPARE_FOR_DEFENSE){
					successQueue.add(actionCreator.createGoStraight(inchesToMeters(40.0), .3, Rotation2.DEG_90, faceAngle));
				} else {
					final boolean hatch;
					if (afterComplete == AfterComplete.GO_TO_LOADING_STATION_HATCH) {
						hatch = true;
					...
					successQueue.add(actionCreator.createGoStraight(inchesToMeters(30), .4, towardsCargoShipAngle.unaryMinus(), faceAngle));
					successQueue.add(actionCreator.createGoStraight(inchesToMeters(100), .4, Rotation2.DEG_270, faceAngle));
					final GamePieceType afterGamePieceType = hatch ? GamePieceType.HATCH : GamePieceType.CARGO;
					final Rotation2 faceDirection = getManipulatorOffset(afterGamePieceType).minusDegrees(90);
			...
			actionQueue.add(actionCreator.createGoStraight(inchesToMeters(69.56 + 6), .3, sideRocketIsLeft ? Rotation2.DEG_180 : Rotation2.ZERO, startingOrientation)); // the 6 is random
			actionQueue.add(actionCreator.createGoStraight(inchesToMeters(201.13 - 95.28) - FieldDimensions.HAB_LIP_DISTANCE - inchesToMeters(60), .5, Rotation2.DEG_90, startingOrientation)); // the 60 is random
			...
			if(lineUpType == LineUpType.USE_VISION) {
				...
				successQueue.add(actionCreator.createGoStraight(inchesToMeters(10), .3, driveAngle.plusDegrees(180), driveAngle));
				successQueue.add(actionCreator.createGoStraight(inchesToMeters(20), .3, towardsCenter, driveAngle));
			} else {
				actionQueue.add(actionCreator.createGoStraight(inchesToMeters(60 - 1), .3, Rotation2.DEG_90, startingOrientation)); // the 3 is random
				actionQueue.add(actionCreator.createTurnToOrientation(driveAngle));
				actionQueue.add(actionCreator.createGoStraight(inchesToMeters(30), .2, driveAngle, driveAngle));
				actionQueue.add(actionCreator.createDropHatch());
				actionQueue.add(actionCreator.createGoStraight(inchesToMeters(10), .2, driveAngle.plusDegrees(180), driveAngle));
				actionQueue.add(actionCreator.createGoStraight(inchesToMeters(20), .3, towardsCenter, driveAngle));
				actionQueue.add(successQueue);
			}
			if(afterComplete != null) {
				if (afterComplete == AfterComplete.PREPARE_FOR_DEFENSE) {
					successQueue.add(actionCreator.createGoStraight(inchesToMeters(40), .3, towardsCenter));
					successQueue.add(actionCreator.createTurnToOrientation(Rotation2.DEG_90));
					successQueue.add(new Actions.ActionMultiplexerBuilder(
							actionCreator.createGoStraight(inchesToMeters(75), .3, Rotation2.DEG_90, Rotation2.DEG_90),
					...
					successQueue.add(actionCreator.createGoStraight(inchesToMeters(30), .3, Rotation2.DEG_270));
					...
					successQueue.add(actionCreator.createGoStraight(inchesToMeters(100), .4, Rotation2.DEG_270, faceDirection));
				}
			}
		} else {
